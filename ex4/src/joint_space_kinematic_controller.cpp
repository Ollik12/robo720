#include "ex4/joint_space_kinematic_controller.hpp"
...
controller_interface::return_type JointSpaceKinematicController::update(
  ...
  const rclcpp::Duration& period) {
     ...
     */
    // input for this controller is the desired pose defined in pose_d 
    // output shoudl be written to low-level controllers interfaces (q_dot_cmd_)
    
    // Looking at the figue 1. we see that to get q_d we should use inverse kinematics solver.
    // and current q is q_kdl_ to be given to the solver.
...
CallbackReturn JointSpaceKinematicController::on_init() {
...
CallbackReturn JointSpaceKinematicController::on_configure(
  const rclcpp_lifecycle::State& /*previous_state*/) {
    ...
    goal_pose_subscriber_ = get_node()->create_subscription<geometry_msgs::msg::PoseStamped>(
        "goal_pose", rclcpp::SystemDefaultsQoS(), std::bind(&JointSpaceKinematicController::goal_pose_callback,
...
JointSpaceKinematicController::command_interface_configuration() const {
...
controller_interface::InterfaceConfiguration
JointSpaceKinematicController::state_interface_configuration() const {
    controller_interface::InterfaceConfiguration config;
    config.type = controller_interface::interface_configuration_type::INDIVIDUAL;
...
CallbackReturn JointSpaceKinematicController::on_activate(
  const rclcpp_lifecycle::State& /*previous_state*/) {
    updateJointStates();
    elapsed_time_ = 0.0;
...
void JointSpaceKinematicController::updateJointStates() {
...
void JointSpaceKinematicController::goal_pose_callback(const std::shared_ptr<geometry_msgs::msg::PoseStamped> msg) {
    rt_goal_pose_buffer_.writeFromNonRT(msg);
}

// Debug print functions to reduce clutter in controller code
void JointSpaceKinematicController::kdl_chain_error_print() {
...
void JointSpaceKinematicController::kdl_chain_success_print() {
...
PLUGINLIB_EXPORT_CLASS(ex4::JointSpaceKinematicController,
