...
: Node("trajectory_publisher")
{
    ...
    // initial trajectory type circle on startup
    trajectory_type_ = this->declare_parameter("trajectory_type", "circle");

    // lets add callback for trajectory_type_ for runtime changing with terminal
    // lets copy other callback functrionality from joint_ctrl and implement here
    // and we already are inherited fron rclcpp::node so let's use this and not get_node()
    parameter_callback_handle_ = this->add_on_set_parameters_callback(
        ...
        for (const auto &param : parameters) {
            if (param.get_name() == "trajectory_type") {
                // Lets store the given parameter in variable and check validity
                const auto new_type = param.as_string();

                if (new_type == "circle" or new_type == "line" or new_type == "ellipse") {
                    // accepted trajectory type -> set and log successfull change
                    trajectory_type_ = new_type;
                    RCLCPP_INFO(
                        this->get_logger(),
                    ...
                    // not known trajectory type gets logged and has no actual change in functionality
                    result.successful = false;
                    result.reason = "Unknown trajectory type: " + new_type;
                    RCLCPP_INFO(
                        this->get_logger(),
                        "Trajectory type is not recognized: %s",
                        trajectory_type_.c_str());
                }
            }
        }
...
void TrajectoryPublisher::circle_trajectory(KDL::Vector& tgt_pos, KDL::Twist& tgt_vel, double t) {
    ...
    tgt_vel.vel.z(0.05 * cos(0.5 * t));

}

// ellipse trajectory function
